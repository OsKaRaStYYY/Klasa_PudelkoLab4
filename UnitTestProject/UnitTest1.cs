...
namespace PudelkoUnitTests
{
    ...
    public class UnitTestsPudelkoConstructors
    {
        ...
        public void Constructor_2params_InMilimeters(double a, double b, double expectedA, double expectedB)
        {
            ...
            AssertPudelko(p, expectedA, expectedB, expectedC: 0.01);
        }
        ...
        #region Parsing =========================================
        [TestMethod]
        public void Parse_ValidInput_ReturnsCorrectPudelko()
        {
            
            string input = "1.2 m × 3.4 m × 5.6 m";
            Pudelko expectedPudelko = new Pudelko(1.2, 3.4, 5.6, UnitOfMeasure.Meter);

            Pudelko parsedPudelko = Pudelko.Parse(input);

         
            Assert.AreEqual(expectedPudelko, parsedPudelko);
        }

        [TestMethod]
        [DataRow(null)]
        [DataRow("")]
        public void Parse_NullOrEmptyInput_ThrowsArgumentNullException(string input)
        {
            
            Assert.ThrowsException<ArgumentNullException>(() => Pudelko.Parse(input));
        }

        [TestMethod]
        [DataRow("1.2 × 3.4 × 5.6")]
        [DataRow("1.2 m 3.4 m 5.6 m")]
        [DataRow("1.2 m, 3.4 m, 5.6 m")]
        public void Parse_InvalidFormatInput_ThrowsFormatException(string input)
        {
        ...
        [TestMethod]
        public void Parse_DifferentUnits_ThrowsFormatException()
        {
            
            string input = "1.2 m × 3.4 cm × 5.6 mm";

            Assert.ThrowsException<FormatException>(() => Pudelko.Parse(input));
        }

        [TestMethod]
        public void Parse_InvalidUnit_ThrowsFormatException()
        {
           
            string input = "1.2 x × 3.4 x × 5.6 x";
